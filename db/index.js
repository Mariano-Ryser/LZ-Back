...
if (!MONGO_PASS) {
  console.error('❌ ERROR: La variable MONGO_DB_PASS no está definida en el archivo .env');
  process.exit(1);
}
...
const dbConnect = (app) => {
  ...
  .then(() => {
    ...
    setTimeout(() => console.log('📦 Base de datos lista (dbConnect.js)'), 500);
  })
  .catch((err) => {
    ...
    if (err.message.includes('bad auth')) {
      ...
      console.error('⛔ Conexión rechazada. Revisa si tu IP está autorizada en MongoDB Atlas.');
    }
